...
def split_sentences(text, ind_sent = False):
    ...
    return [undo_replacement(sentence) for sentence in get_sentences(processed, ind_sent)]
...
def get_sentences(text, ind_sent = False):
...
def clean_text_by_sentences(text):
    ...
    """
    original_sentences = split_sentences(text, ind_sent = False)
